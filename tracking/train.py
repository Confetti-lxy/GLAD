...
def parse_args():
    ...
    parser.add_argument('--mode', type=str, choices=["single", "multiple", "deepspeed"], default="multiple",
                        help="train on single gpu, multiple gpus or deepspeed")
    ...
    # for deepspeed
    parser.add_argument('--gpu_vis', type=str, default='0')
    parser.add_argument('--config_file', type=str, default='experiments/ds_config.json')
...
def main():
    ...
    if args.mode == "single":
        ...
        train_cmd = "deepspeed --include localhost:%s --master_port %s lib/train/run_training_ds.py " \
                    "--deepspeed --deepspeed_config %s --script %s --config %s --save_dir %s " \
                    "--precision %s --use_lmdb %d --script_prv %s --config_prv %s --stage1_model %s" \
                    % (args.gpu_vis, args.master_port,
                    args.config_file, args.script, args.config, args.save_dir, 
                    args.precision, args.use_lmdb, args.script_prv, args.config_prv, args.stage1_model)
    else:
        raise ValueError("mode should be 'single', 'multiple' or 'deepspeed'.")
    os.system(train_cmd)
