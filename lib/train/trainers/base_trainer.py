...
class BaseTrainer:
    ...
    def load_deepspeed_checkpoint(self, mode='latest', fields=['epoch']):
        if mode is 'latest':
            checkpoint_path = '{}/deepspeed'.format(self._checkpoint_dir)
            if os.path.exists(checkpoint_path):
                print(f"Loading deepspeed checkpoint from {checkpoint_path}")
                self.actor.net.load_checkpoint(checkpoint_path)
                # Currently, we need to set epoch manually when resume training
                self.epoch =82
        ...
        for i in range(num_tries):
            try:
                if load_latest:
                    ...
                    self.load_deepspeed_checkpoint(mode='latest')
                ...
                for epoch in tqdm(range(self.epoch+1, max_epochs+1)):
                    self.epoch = epoch
                    ...
                    # if epoch > (max_epochs - 10) or save_every_epoch or epoch % 20 == 0 or epoch == 1:
                    if epoch % 40 == 0 or epoch == max_epochs or epoch == 1:
                        if self._checkpoint_dir:
                            if self.settings.local_rank in [-1, 0]:
                                self.save_checkpoint()
                            if self.settings.backend == "deepspeed":
                                directory = '{}/{}'.format(self._checkpoint_dir, self.settings.backend)
                                if self.settings.local_rank in [-1, 0]:
                                    if not os.path.exists(directory):
                                        print(f"{directory} doesn't exist. creating...")
                                        os.makedirs(directory)
                                self.actor.net.save_checkpoint(directory, client_state={'epoch': self.epoch})
