...
class FocalLoss(nn.Module, ABC):
    ...
    def forward(self, prediction, target):
        ...
        positive_loss = torch.log(prediction + self.epsilon) * torch.pow(1 - prediction, self.alpha) * positive_index
        negative_loss = torch.log(1 - prediction + self.epsilon) * torch.pow(prediction,
